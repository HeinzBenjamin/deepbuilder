...
class DeepBuilderEnv(gym.GoalEnv):
    def __init__(self, sess_name, act_dim, box_dim, max_num_boxes, height_field_res):
        ...
        self.observation = spaces.Box(low=0.0, high=1600.0, shape=(height_field_res * height_field_res,), dtype=np.float64)
        self.desired_goal = spaces.Box(0.0, 1600.0, shape=(height_field_res * height_field_res,), dtype=np.float64)
        self.achieved_goal = spaces.Box(0.0, 1600.0, shape=(height_field_res * height_field_res,), dtype=np.float64)
    ...
    def step(self, action):
        ...
        if not self.use_moveit:
            ...
            ppp = []
            for i in range(6):
                ppp.append(float(action[i]))
            pose_reachable, self.path = self.ros_comm.test_pose(ppp)
    ...
    def reset(self):
        ...
        if self.fh_needs_reset or self.path == [] or self.has_boxes:
            self.FH_Reset()
        ...
        #init observation with very small values, which should be interpreted as "no boxes present"
        #if boxes are added to the scene, their vals will get quite high and thus actually start filling obs in random order 
