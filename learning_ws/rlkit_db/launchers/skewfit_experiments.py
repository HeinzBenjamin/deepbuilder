...
from rlkit_db.core import logger
from rlkit_db.envs.vae_wrapper import temporary_mode

import cv2
import numpy as np
import os.path as osp

from rlkit_db.samplers.data_collector.vae_env import (
    VAEWrappedEnvPathCollector,
)
from rlkit_db.torch.her.her import HERTrainer
from rlkit_db.torch.sac.policies import MakeDeterministic
from rlkit_db.torch.sac.sac import SACTrainer
from rlkit_db.torch.skewfit.online_vae_algorithm import OnlineVaeAlgorithm
from rlkit_db.util.io import load_local_or_remote_file
from rlkit_db.util.video import dump_video
...
def train_vae(variant, return_data=False):
    from rlkit_db.util.ml_util import PiecewiseLinearSchedule
    from rlkit_db.torch.vae.conv_vae import (
        ConvVAE,
    )
    import rlkit_db.torch.vae.conv_vae as conv_vae
    from rlkit_db.torch.vae.vae_trainer import ConvVAETrainer
    from rlkit_db.core import logger
    import rlkit_db.torch.pytorch_util as ptu
    from rlkit_db.pythonplusplus import identity
    import torch
    beta = variant["beta"]
    ...
    if dataset_path is not None:
        ...
        if use_cached and osp.isfile(filename):
            ...
            if random_rollout_data:
                from rlkit_db.exploration_strategies.ou_strategy import OUStrategy
...
def get_envs(variant):
    from multiworld.core.image_env import ImageEnv
    from rlkit_db.envs.vae_wrapper import VAEWrappedEnv
    from rlkit_db.util.io import load_local_or_remote_file
...
def get_exploration_strategy(variant, env):
    from rlkit_db.exploration_strategies.epsilon_greedy import EpsilonGreedy
    from rlkit_db.exploration_strategies.gaussian_strategy import GaussianStrategy
    from rlkit_db.exploration_strategies.ou_strategy import OUStrategy
...
def skewfit_experiment(variant):
    import rlkit_db.torch.pytorch_util as ptu
    from rlkit_db.data_management.online_vae_replay_buffer import \
        OnlineVaeRelabelingBuffer
    from rlkit_db.torch.networks import FlattenMlp
    from rlkit_db.torch.sac.policies import TanhGaussianPolicy
    from rlkit_db.torch.vae.vae_trainer import ConvVAETrainer
